...
def combine_images(indices, tx, ty):
    if len(set([ty[i] for i in indices])) < len(indices):
        raise ValueError(f"Labels must all be unique, {indices}")
    return np.mean(tx[indices], dtype=np.uint8, axis=0)
...
def make_data(ndata, tx, ty, max_iter=1000):
    ...
    ims = np.zeros((ndata,28,28), dtype=np.uint8)
    labels = np.zeros((ndata,10), dtype=np.uint8)
    i = 0
    choices = set()
    choice = np.random.choice(all_idx, size=2, replace=2)
    try:
        while i < ndata:
            j = 0
            choice = tuple(choice)
            while choice in choices:
                choice = tuple(np.random.choice(all_idx, size=2, replace=2))
                j += 1
                if j >= max_iter:
                    raise BadTime
            choices.add(choice)
